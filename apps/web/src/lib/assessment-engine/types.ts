...
export const answerAsString = (value: AnswerValue): string => (typeof value === 'string' ? value : '');

/** Safely read an answer value as a string array (returns [] if string or undefined). */
export const answerAsArray = (value: AnswerValue): string[] => (Array.isArray(value) ? value : []);
