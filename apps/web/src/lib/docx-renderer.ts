...
function numberedItems(items: string[], ref: string, fontSize = 19): Paragraph[] {
  return items.map(
    (item) =>
      new Paragraph({
...
function buildNarrative(doc: PdfReportDocument): (Paragraph | Table)[] {
  ...
  els.push(
    note('System-generated explanatory text. Review against the record facts and cited sources before reliance.'),
  );
...
function buildIssueBlock(issue: PdfOpenIssue, index: number): (Paragraph | Table)[] {
  ...
  els.push(
    new Paragraph({
      ...
      border: index > 0 ? { top: { style: BorderStyle.SINGLE, size: 2, color: C.border, space: 8 } } : undefined,
      children: [tag(`Issue ${index + 1}`), new TextRun({ text: '  ', font: FONT, size: 15 }), tag(kindLabel)],
    }),
  );
...
function buildAltPathways(doc: PdfReportDocument): (Paragraph | Table)[] {
  ...
  return [
    ...
    ...numberedItems(
      doc.alternativePathways.map((ap) => ap.description),
      'main-numbered',
    ),
  ];
}
...
export async function generateAndDownloadDocx(reportDoc: PdfReportDocument): Promise<void> {
  ...
  const idPart = reportDoc.header.assessmentId ? `-${reportDoc.header.assessmentId}` : '';
  const namePart = reportDoc.header.assessmentName
    ? `-${reportDoc.header.assessmentName
        .replace(/[^a-zA-Z0-9]+/g, '-')
        .replace(/-+$/, '')
        .slice(0, 40)}`
    : '';
